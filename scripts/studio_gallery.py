            ...
            for dirpath, dirnames, filenames in os.walk(root):
                ...
                for filename in filenames:
                    ...
                    cur = db.execute(
                        ...
                        "(filename,folder,filepath,width,height,file_date,search_text,media_type) "
                        "VALUES (?,?,?,?,?,?,?,?)",
                        (filename, display_folder, filepath, w, h, file_date, search, media_type),
                    )
                    iid = cur.lastrowid
                    if iid == 0:
                        continue
...
def save_metadata_for_image(filepath, infotext, settings=None):
    """
    Save generation metadata to Gallery DB for an image.

    Called directly from studio_api.py after saving an image to disk.
    If the image hasn't been scanned into Gallery yet, the save is skipped
    silently — the embedded file metadata serves as fallback until the next scan.

    Args:
        filepath: Full path to the saved image file
        infotext: A1111-format parameters string (from processed.infotexts)
        settings: Optional dict with structured generation params
    """
    try:
        db = _get_db()
    except Exception:
        return  # Gallery DB not initialized yet
    try:
        ...
        if not row:
            return  # Image not in Gallery DB yet — embedded metadata is fallback

        image_id = row["id"]

        # Parse structured fields from infotext if settings not provided
        meta = {}
        if settings and isinstance(settings, dict):
            meta = dict(settings)
        if infotext:
            parsed = parse_sd_parameters(infotext)
            # Infotext fields fill gaps but don't override settings
        ...
        """, (
            ...
            meta.get("raw_infotext") or infotext,
            time.time(),
        ))
        db.commit()
    except Exception as e:
...
def setup_gallery_routes(app: FastAPI):
    ...
    async def gallery_image_metadata(image_id: int):
        ...
        try:
            ...
            if meta and meta.get("prompt") or meta.get("raw_parameters"):
                ...
                return meta
            # Fall back to DB-stored metadata
            db_meta = db.execute("SELECT * FROM image_metadata WHERE image_id=?", (image_id,)).fetchone()
            if db_meta:
                result = {k: db_meta[k] for k in db_meta.keys() if db_meta[k] is not None and k != "image_id"}
    ...
    async def gallery_save_metadata(request: Request):
        """Save generation metadata to Gallery DB for an image by filepath."""
        data = await request.json()
        filepath = data.get("filepath", "").strip()
        metadata = data.get("metadata", {})
        if not filepath or not metadata:
            return JSONResponse({"error": "filepath and metadata required"}, status_code=400)
        db = _get_db()
        try:
            row = db.execute("SELECT id FROM images WHERE filepath=?", (filepath,)).fetchone()
            if not row:
                # Image not yet scanned — try normalizing path
                norm = os.path.normpath(filepath)
                row = db.execute("SELECT id FROM images WHERE filepath=?", (norm,)).fetchone()
            if not row:
                return {"saved": False, "reason": "image_not_found"}
            image_id = row["id"]
            db.execute("""
                INSERT OR REPLACE INTO image_metadata (
                    image_id, prompt, negative_prompt, seed, steps, cfg,
            ...
            """, (
                ...
                metadata.get("seed"),
                metadata.get("steps"),
                metadata.get("cfg") or metadata.get("cfg_scale"),
                metadata.get("sampler") or metadata.get("sampler_name"),
                metadata.get("scheduler") or metadata.get("schedule_type"),
                metadata.get("model") or metadata.get("sd_model"),
                metadata.get("model_hash") or metadata.get("sd_model_hash"),
                metadata.get("denoising") or metadata.get("denoising_strength"),
                metadata.get("width"),
                metadata.get("height"),
                metadata.get("clip_skip"),
                metadata.get("hires_upscaler") or metadata.get("hr_upscaler"),
                metadata.get("hires_upscale") or metadata.get("hr_scale"),
                metadata.get("raw_infotext") or metadata.get("infotexts"),
                time.time(),
            ))
            db.commit()
    ...
    async def gallery_restore(trash_id: int):
        ...
        try:
            ...
            cur = db.execute(
                "INSERT INTO images (filename,folder,filepath,width,height,file_date,search_text,media_type) "
                "VALUES (?,?,?,?,?,?,?,?)",
                (ofn, row["original_folder"], ofp, row["width"], row["height"],
                 row["file_date"], row["search_text"], _mt),
            )
    ...
    async def gallery_bulk_copy(request: Request):
        ...
        try:
            ...
            for img_id in ids:
                ...
                cur = db.execute(
                    "INSERT INTO images "
                    "(filename,folder,filepath,width,height,file_date,search_text,media_type,phash) "
                    "VALUES (?,?,?,?,?,?,?,?,?)",
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    db.execute(
                        "UPDATE images SET search_text=?, phash=? WHERE id=?",
                        (new_search, new_hash, image_id),
                    )
                    db.commit()
                except Exception:
                    pass
                ...
                if keep_original:
                    ...
                    cur = db.execute(
                        ...
                        (new_fn, display_folder, dst_fp, new_w, new_h,
                         new_mtime, new_search, "image", new_hash),
                    )
                    new_img_id = cur.lastrowid
                    new_ids.append(new_img_id)
                    char_rows = db.execute(
                    ...
                    db.execute(
                        ...
                        "search_text=?, phash=? WHERE id=?",
                        (new_fn, dst_fp, new_w, new_h, new_search, new_hash, img_id),
                    )
                converted += 1
