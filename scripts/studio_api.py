...
def _pil_to_preview_b64(img: Image.Image) -> str:
    """Fast preview encoder — JPEG at q=55 is ~10x faster than PNG and
    produces ~5x smaller payloads. Only used for live preview thumbnails
    during generation, not for final output images."""
    buf = io.BytesIO()
    if img.mode == "RGBA":
        img = img.convert("RGB")
...
def _progress_polling_thread():
    ...
    _last_preview_time = [0.0]  # timestamp of last VAE decode
    _PREVIEW_MIN_INTERVAL = 1.5  # minimum seconds between VAE decodes
    ...
    while True:
        ...
        try:
            ...
            preview_b64 = None
            # Only decode a new preview when the step has actually advanced
            # AND enough time has passed since the last decode. This prevents
            # set_current_image() (which runs a VAE decode on the GPU) from
            # stalling the sampler via CUDA synchronization every 400ms.
            now = time.time()
            current_key = (job_no, sampling_step)
            step_changed = current_key != _last_preview_step[0]
            interval_ok = (now - _last_preview_time[0]) >= _PREVIEW_MIN_INTERVAL
            ...
            if should_decode:
                try:
                    ...
                    if current_img:
                        # FR-029: Cap shortest edge at 240px for preview thumbnail
                    ...
                    # Preview fetch failed — non-fatal, use cached preview
                    if not _logged_error[0]:
                        print(f"{TAG} Preview fetch error (non-fatal, using cache): {e}")
    ...
    async def load_vae(body: dict):
        ...
        try:
            ...
            if hasattr(shared.sd_model, 'first_stage_model'):
                sd_vae.reload_vae_weights(shared.sd_model, vae_name)
