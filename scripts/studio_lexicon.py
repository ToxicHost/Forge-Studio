...
def setup_lexicon_routes(app: FastAPI):
    ...
    async def lexicon_file_delete(path: str, force: bool = False):
        try:
            full = _safe_path(path)
        except ValueError as e:
            return JSONResponse({"error": str(e)}, status_code=400)
        ...
        if os.path.isdir(full):
            ...
            if contents and not force:
                # Count files recursively for the confirmation dialog
                file_count = 0
                for _root, _dirs, _files in os.walk(full):
                    file_count += sum(1 for f in _files if f.endswith(".txt"))
                return JSONResponse({
