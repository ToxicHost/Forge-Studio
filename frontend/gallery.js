...
async function loadMetadata(img) {
    ...
    try {
        ...
        for (const [k, l0] of ord) { if (m[k] != null && m[k] !== "") { has = true; const l = (k === "denoising") ? denoiseLabel : l0; h += '<div class="gal-meta-row"><div class="gal-meta-label">' + l + '</div><div class="gal-meta-value' + (k === "prompt" || k === "negative_prompt" ? " prompt" : "") + '">' + esc(String(m[k])) + '</div></div>';
            if (k === "prompt" && resolvedPrompt && resolvedPrompt.trim() !== String(m[k]).trim()) { h += '<div class="gal-meta-row"><div class="gal-meta-label">Resolved</div><div class="gal-meta-value prompt">' + esc(resolvedPrompt) + '</div></div>'; }
        } }
...
async function renderSettingsView() { await loadIgnoreWords(); const spv = localStorage.getItem("gal_send_prompt_version") || "resolved"; return '<div class="gal-settings"><div class="gal-settings-title">Gallery Settings</div><div class="gal-settings-card"><h3>Send to Canvas</h3><p>Which prompt version to apply when sending params from an image back to the canvas.</p><label style="display:flex;align-items:center;gap:8px;margin:4px 0;font-size:12px;color:var(--text-2);cursor:pointer"><input type="radio" name="gal-send-prompt-ver" value="resolved"' + (spv === "resolved" ? " checked" : "") + '/> Resolved (what the model saw)</label><label style="display:flex;align-items:center;gap:8px;margin:4px 0;font-size:12px;color:var(--text-2);cursor:pointer"><input type="radio" name="gal-send-prompt-ver" value="raw"' + (spv === "raw" ? " checked" : "") + '/> Raw (with wildcards)</label></div><div class="gal-settings-card"><h3>Ignore Words</h3><p>Words to skip when parsing character tags from filenames.</p><div style="margin-bottom:8px;display:flex;flex-wrap:wrap;align-items:center;gap:0">' + G.ignoreWords.map(w => '<span class="gal-ignore-word-tag">' + esc(w) + ' <span class="remove" data-remove-word="' + esc(w) + '">\u00d7</span></span>').join("") + (!G.ignoreWords.length ? '<span style="color:var(--text-4);font-size:11px">None.</span>' : "") + '<button class="gal-add-tag-btn" id="gal-ignore-toggle" style="margin:2px">+</button><div class="gal-add-tag-inline" id="gal-ignore-inline" style="min-width:100px;margin:2px"><input id="gal-ignore-input" placeholder="Word..." /></div></div><p style="margin-top:4px;font-size:10px;color:var(--text-4)">After changes, re-scan to apply.</p></div><div class="gal-settings-card"><h3>Tagging Rules</h3><div style="font-size:11px;color:var(--text-2);line-height:1.8"><strong>Character separators:</strong> comma, plus (+)<br><strong>Name connectors:</strong> space, underscore, dash (joined into one name)<br><strong>Filtered:</strong> (...), [...], trailing numbers, numeric strings, ignore words<br><strong>Min length:</strong> 2+ characters with a letter<br><strong>Manual:</strong> add/remove tags per image in detail view</div></div></div>'; }
