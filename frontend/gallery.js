...
function _wireDetailEvents(ov, img) {
    ...
    ov.addEventListener("click", e => { const a = e.target.closest("[data-action]"); if (!a) return; const act = a.dataset.action, id = parseInt(a.dataset.imgId);
        ...
        else if (act === "send-canvas-menu") { const r = a.getBoundingClientRect(); _buildCtxMenu([{ icon: IC.canvas, label: "Send to Canvas — raw prompt", fn: () => sendToCanvas(id, "raw") }, { icon: IC.canvas, label: "Send to Canvas — resolved prompt", fn: () => sendToCanvas(id, "resolved") }], r.left, r.bottom + 2); }
